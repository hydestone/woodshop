    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display:'flex', gap:12, padding:'0 16px 16px', flexWrap:'wrap' }}>
        {[
          ...
          <div key={s.label} style={{ flex:1, minWidth:100, background:'var(--c-bg-surface)', borderRadius:12, padding:'12px 16px', border:'1px solid var(--c-border-light)', boxShadow:'var(--shadow-sm)' }}>
            <div style={{ fontSize:11, color:'var(--c-text-faint)', fontWeight:600, textTransform:'uppercase', letterSpacing:'.5px', marginBottom:4 }}>{s.label}</div>
      ...
      {/* Tabs */}
      <div style={{ display:'flex', borderBottom:'1px solid var(--c-border-light)', padding:'0 16px', gap:0 }}>
        {[['projects','By Project'],['shop','General Shop']].map(([id,label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ padding:'10px 16px', background:'none', border:'none', borderBottom: tab===id ? '2px solid var(--accent)' : '2px solid transparent', color: tab===id ? 'var(--accent)' : 'var(--c-text-muted)', fontWeight: tab===id ? 700 : 400, fontSize:14, cursor:'pointer', fontFamily:'inherit', marginBottom:-1 }}>
            {label}
          </button>
        ))}
      </div>
        ...
        {tab === 'projects' && (
          projectCosts.length === 0 ? (
            <div className="empty" style={{ paddingTop:40 }}>
              <div className="empty-icon"><IDollar size={32} color="var(--c-text-muted)" sw={1.5} /></div>
            ...
            <div className="group" style={{ marginTop:8 }}>
              {projectCosts.map((p, i) => (
                <div key={p.id} style={{ padding:'14px 16px', borderBottom: i < projectCosts.length-1 ? '1px solid var(--c-border-light)' : 'none' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    <div>
                      <div style={{ fontWeight:600, fontSize:15 }}>{p.name}</div>
                      <div style={{ fontSize:12, color:'var(--c-text-faint)', marginTop:2 }}>{p.itemCount} item{p.itemCount!==1?'s':''} · {p.category || 'No category'} · {p.status}</div>
                    </div>
                    <div style={{ fontWeight:800, fontSize:16, color:'var(--accent)' }}>{fmt(p.total)}</div>
                  </div>
                  {p.items.length > 0 && (
                    <div style={{ marginTop:8, paddingLeft:8, borderLeft:'2px solid var(--c-border-light)' }}>
                      {p.items.map(item => (
                        <div key={item.id} style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'var(--c-text-muted)', padding:'2px 0' }}>
                          <span>{item.name}</span>
          ...
          shopItems.length === 0 ? (
            ...
            <div className="group" style={{ marginTop:8 }}>
              {shopItems.map((item, i) => (
                <div key={item.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 16px', borderBottom: i < shopItems.length-1 ? '1px solid var(--c-border-light)' : 'none' }}>
                  <div>
                    <div style={{ fontWeight:500, fontSize:14 }}>{item.name}</div>
                    {item.store && <div style={{ fontSize:12, color:'var(--c-text-faint)' }}>{item.store}</div>}
                  </div>
