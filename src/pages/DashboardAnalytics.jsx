// ─── Dashboard analytics ──────────────────────────────────────────────────────
// Lazy-loaded from Dashboard.jsx so the ECharts chunk (~1.1 MB) is fetched only
// when the Analytics section scrolls into view — never on initial app load.
        ...
        {carouselPhotos.length > 1 && (
          <div style={{ display: 'flex', gap: 6, marginTop: 10, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 2 }}>
            {carouselPhotos.map((ph, i) => (
              <PhotoImg key={ph.id} photo={ph} alt="" onClick={() => setCur(i)} loading="lazy"
...
// ── Section ───────────────────────────────────────────────────────────────────
export default function AnalyticsSection({ data, categories, locations, isDark, onDrill, onLocationClick }) {
  return (
    <div className="dash-grid">
      <div className="card"><div className="label-caps-sm">Projects by Year</div><ProjectsByYear projects={data.projects} photos={data.photos} onDrill={onDrill} isDark={isDark} /></div>
      <div className="card"><div className="label-caps-sm">Species Breakdown</div><SpeciesDonut projects={data.projects} onDrill={onDrill} isDark={isDark} /></div>
      <div className="card"><div className="label-caps-sm">Category Heatmap</div><CategoryHeatmap projects={data.projects} categories={categories} onDrill={onDrill} isDark={isDark} /></div>
      <div className="card"><div className="label-caps-sm">Finish Usage</div><FinishUsage projects={data.projects} onDrill={onDrill} isDark={isDark} /></div>
      <div className="card"><div className="label-caps-sm">Wood Source Map</div><WoodSourceMap locations={locations} woodStock={data.woodStock} projectWoodSources={data.projectWoodSources} onLocationClick={onLocationClick} /></div>
      <div className="card" style={{gridColumn:'1/-1'}}><div className="label-caps-sm">Material Flow — Species → Category → Finish</div><MaterialFlow projects={data.projects} isDark={isDark} /></div>
    </div>
  )
}
