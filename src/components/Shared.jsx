  ...
  return createPortal(
    ...
    >
      <div ref={sheetRef} className={`sheet${variant === 'form' ? ' sheet-form' : ''}`}>
        ...
        <div className="sheet-body" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>{children}</div>
      </div>
    </div>,
    document.body
  )
...
export function Lightbox({ photos, index, onClose, onEdit }) {
  ...
  useEffect(() => { reset(); setRotation(0); setSavingRotation(false) }, [cur])

  const handleClose = () => {
    if (rotation !== 0 && !savingRotation) {
  ...
  return createPortal(
    ...
    >
      ...
      {savingRotation && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, zIndex: 10 }}>
          <p style={{ color: '#fff', fontSize: 16, fontWeight: 600, textAlign: 'center', margin: 0 }}>Save rotated photo?</p>
          <p style={{ color: 'rgba(255,255,255,.6)', fontSize: 13, textAlign: 'center', margin: 0 }}>This will update the saved photo.</p>
          <div style={{ display: 'flex', gap: 12 }}>
            <button type="button" onClick={() => { setSavingRotation(false); onClose() }}
              style={{ padding: '10px 24px', borderRadius: 10, border: '1px solid rgba(255,255,255,.3)', background: 'transparent', color: '#fff', fontSize: 15, cursor: 'pointer', fontFamily: 'inherit' }}>
              Don't Save
            </button>
            <button type="button" onClick={async () => {
              const photo = photos[cur]
              if (onEdit && photo) {
                await onEdit(photo, { rotation: ((photo.rotation || 0) + rotation + 360) % 360 })
              }
              setSavingRotation(false)
              onClose()
            }}
              style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              Save
            </button>
          </div>
        </div>
