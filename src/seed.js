...
export async function seedSampleData() {
  ...
  await supabase.from('categories').insert(catRows).catch(() => {})
  ...
  await supabase.from('resources').insert(resRows).catch(() => {})

  // 3. Wood stock
  ids.woodStockId = uid()
  await supabase.from('wood_stock').insert({
  ...
  // 6. Coat
  ids.coatId = uid()
  await supabase.from('coats').insert({
  ...
  ids.pwsId = uid()
  await supabase.from('project_wood_sources').insert({
  ...
  await supabase.from('shopping').insert(shopRows)
...
// ── Clear sample data ────────────────────────────────────────────────────────
// Removes sample project, steps, coats, photos, shopping, wood stock
// Keeps categories and resources (they're useful starting points)
...
export async function clearSampleData() {
  ...
  if (ids.photoId) {
    if (ids.photoStoragePath) {
      await supabase.storage.from(BUCKET).remove([ids.photoStoragePath]).catch(() => {})
    }
    await supabase.from('photos').delete().eq('id', ids.photoId).catch(() => {})
  }

  // Steps
  if (ids.stepIds?.length) {
    await supabase.from('steps').delete().in('id', ids.stepIds).catch(() => {})
  }

  // Coat
  if (ids.coatId) {
    await supabase.from('coats').delete().eq('id', ids.coatId).catch(() => {})
  }

  // Shopping
  if (ids.shopIds?.length) {
    await supabase.from('shopping').delete().in('id', ids.shopIds).catch(() => {})
  }

  // Project → wood source junction
  if (ids.pwsId) {
    await supabase.from('project_wood_sources').delete().eq('id', ids.pwsId).catch(() => {})
  }

  // Project
  if (ids.projectId) {
    await supabase.from('projects').delete().eq('id', ids.projectId).catch(() => {})
  }

  // Wood stock
  if (ids.woodStockId) {
    await supabase.from('wood_stock').delete().eq('id', ids.woodStockId).catch(() => {})
  }
